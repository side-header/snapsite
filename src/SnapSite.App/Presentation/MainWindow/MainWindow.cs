...
using SiteSnap.Presentation.Services;
...
public sealed partial class MainWindow : Window
{
    ...
    private async Task OpenFolderAsync()
    {
        ...
        try
        {
            ...
            var loaded = await Task.Run(() => workspaceService.Open(rootDir));
    ...
    {
        var absolutePath = WorkspacePath.ToAbsolutePath(state.RootDir, relativePath);
        var image = new Image
        {
            MinHeight = 260,
    ...
    private bool SaveToMetadata(string successMessage, bool refreshAfterSave)
    {
        ...
        try
        {
            workspaceService.Save(state.RootDir, state);
            if (refreshAfterSave)
            {
                scanResult = workspaceService.Scan(state.RootDir);
                RefreshAll();
            }

            var path = workspaceService.MetadataPath(state.RootDir);
    ...
    private async Task ExportOneAsync(ExportFormat format)
    {
        ...
        try
        {
            ...
            {
                documentExportService.ExportHwpxTo(state.RootDir, outputPath, state);
            }
            else
            {
                documentExportService.ExportDocxTo(state.RootDir, outputPath, state);
            }
